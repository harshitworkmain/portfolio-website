...
6. **Telemetry Achievements Timeline (`/achievements`):** An interactive vertical timeline featuring a dynamic sine-wave SVG path that fills smoothly on scroll (using Framer Motion `useScroll` + `useSpring`). Features a hybrid alternating grid layout (3-column grid on desktop, offset margins on mobile) where telemetry node dots are mathematically locked to the wave's peaks. Displays chronologically sorted events with custom monospace telemetry headers (containing date, scope, and status details), inline multi-media galleries with team/group photos as primary featured thumbnails, and a high-contrast dark Lightbox overlay modal supporting full keyboard and mouse navigation. Includes direct "View Post on LinkedIn" action buttons referencing the exact event posts.
7. **Redesigned Projects Subpage (`/projects`):** Chronologically ordered engineering feats prioritizing AI/ML, Robotics, and IoT systems. Cards feature looping, muted HTML5 autoplay video previews on hover with diagnostic telemetry overlays. Projects with missing cover media display a custom `[ COVER PENDING ]` fallback. Features a tabbed details **Specs Modal Console**:
   * **Tab 1: Architecture**: Custom SVG system flowcharts written in React, showing data flow pipelines on a slate grid.
   * **Tab 2: Media**: Screenshot gallery and full video previews.
   * **Tab 3: Arsenal**: Resume-synced technical stack tags and direct links to GitHub code, live apps, and demo files.
8. **Mobile-Responsive Adaptations:** Responsive scaling (`0.65×` scale and vertical offset translation) for both the Three.js 3D backdrop scene and the 2D canvas fallback, as well as responsive text sizing, whitespace-wrapping safeguards for terminal feeds, and dynamically increased glass panel opacities for enhanced mobile text legibility.
